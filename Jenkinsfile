...
def appSourceRepo = 'https://github.com/huanpham15101999/demo-app'
def appSourceBranch = 'master'

def appConfigRepo = 'https://github.com/huanpham15101999/app-helmchart'
def appConfigBranch = 'master'
def helmRepo = "app-helmchart"
def helmChart = "app-demo"
def helmValueFile = "app-demo/app-demo-value.yaml"

def dockerhubAccount = 'dockerhub-credential'
def githubAccount = 'github-credential'
...
pipeline {
    ...
    environment {
        ...
        DOCKER_IMAGE_NAME = "huanpd/demo-app"
        ...
        stage('Update value in helm-chart') {
            steps {
				...
				sh """#!/bin/bash
					   ...
					   git add . ; git commit -m "Update to version ${version}";git push https://${GIT_USERNAME}:${GIT_PASSWORD}@github.com/huanpham15101999/app-helmchart.git
					   cd ..
					   [[ -d ${helmRepo} ]] && rm -r ${helmRepo}
					   """		
				}				
